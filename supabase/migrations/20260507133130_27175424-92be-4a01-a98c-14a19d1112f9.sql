...
BEGIN
  ...
  UNION ALL
  (
    -- Comments: jsonb content; cast to text for substring search
    SELECT 'comment'::text, cm.task_id, coalesce(t.title, 'Comment'),
           left(cm.content::text, 200), t.project_id,
           (similarity(left(cm.content::text, 500), _q)
    ...
    WHERE cm.workspace_id = _workspace_id
      AND (_project_id IS NULL OR t.project_id = _project_id)
      AND cm.content::text ILIKE needle
    ORDER BY rank DESC NULLS LAST
    LIMIT _limit
  )
  UNION ALL
  (
    -- Meetings: search title + raw transcript text + summary jsonb
    SELECT 'meeting'::text, m.id, coalesce(m.title, 'Meeting'),
           left(coalesce(m.transcript_raw_text, m.summary::text, ''), 200), m.project_id,
           (greatest(
              similarity(coalesce(m.title,''), _q),
              similarity(left(coalesce(m.transcript_raw_text,''), 1000), _q) * 0.6,
              similarity(coalesce(m.summary::text,''), _q) * 0.5
            )
    ...
    WHERE m.workspace_id = _workspace_id
      ...
      AND (
        ...
        OR coalesce(m.transcript_raw_text,'') ILIKE needle
        OR coalesce(m.summary::text,'') ILIKE needle
      )
    ORDER BY rank DESC NULLS LAST
    LIMIT _limit
  );
